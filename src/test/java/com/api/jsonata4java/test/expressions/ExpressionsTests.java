...
public class ExpressionsTests {
	...
	public static Collection<Object[]> data() {
		return Arrays.asList(new Object[][] {
			...
			{"[1..2, 3, 2+2]", "[1,2,3,4]", null}, // // mix of sequences and other types of expression
			...
			{"{\"a\":1}.a", "1", null}, //
			{"{\"a\":{\"b\":1}}.a.b", "1", null}, //
			{"{\"a\":null}.a", "null", null}, // // path that resolves to null (different to an unresolvable path)
			{"{\"a\":1}.z", null, null}, // // unresolvable paths
			...
			{"[ [{\"a\":1}] ].a", "1", null}, //
			{"[ [{\"a\":1}, {\"a\":2}] ].a", "[1,2]", null}, //
			{"[ [{\"a\":1}, {\"a\":1}] ].a", "[1,1]", null}, // // check that dupes are not suppressed
			{"[[ {\"a\":{\"b\":1}}, {\"a\":{\"b\":2}} ]].a", "[{\"b\":1}, {\"b\":2}]", null}, //
			{"[[ {\"a\":{\"b\":1}}, {\"a\":{\"b\":2}} ]].a.b", "[1,2]", null}, //
			{"[ [{\"a\":1}], [{\"a\":2}] ].a", "[1,2]", null}, //
			{"[ [{\"a\":1}], [{\"a\":2}] ].z", null, null}, // // unresolvable path
			{"[ [{\"a\":1}], [{\"a\":1}] ].a", "[1,1]", null}, // // check that dupes are not suppressed
			
			{"[ [{\"a\":null}] ].a", "null", null}, // // path on arrays that resolves to null (different to unresolvable paths)
			{"[ [{\"a\":null}], [{\"a\":null}] ].a", "[null, null]", null}, // // path on arrays that resolves to null (different to unresolvable paths)
			{"null.a", null, ExpressionsVisitor.ERR_MSG_INVALID_PATH_ENTRY}, //
			
			
			
			// verify selector result semantics
			{"[{\"a\":[1,2]}, {\"a\":[3,4]}].a", "[1,2,3,4]", null}, // // path resolves to an object in the top-level array (NOTE: not [ [1,2],[3,4] ])
			
			{"[{\"a\":{\"b\":[1,2]}}, {\"a\":{\"b\":[3,4]}}].a.b", "[1,2,3,4]", null}, //
			{"([{\"a\":[1,2]}, {\"a\":[3,4]}]).a", "[1,2,3,4]", null}, // parenthesis do not suppress this behaviour
			
			// same as the above, but the path has square brackets around it
			{"[{\"a\":[1,2]}, {\"a\":[3,4]}].[a]", "[[1,2],[3,4]]", null}, // top-level
			...
			{"[1,2,3][[0,1]]", "[1,2]", null}, //
			{"[1,2,3][[1,0]]", "[1,2]", null}, // // check ordering of input array is retained
			{"[1,2,3][[0,0,1,1,2,2]]", "[1,1,2,2,3,3]", null}, // // check dupes are not suppressed
			...
			{"-22", "-22", null}, //
			{"-22.2", "-22.2", null}, //
			{"-xxx", null, null}, // // NOTE: Javascript JSONata engine actually throws an exception here (but it shouldn't.. this is a bug in that engine that will be fixed)
